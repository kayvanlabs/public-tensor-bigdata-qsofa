function [qrs_amp_raw,qrs_i_raw,p_amp,p_ind,q_amp,q_ind,s_amp,s_ind,t_amp,t_ind,delay,ecg_h]=pan_tompkin(ecg,fs,gr)
...
% p_amp : amplitude of P waves 
% p_ind : index of P waves
% q_amp: amplitude of Q waves
% q_ind: index of Q waves
% s_amp: amplitude of S waves
% s_ind: index of S waves
% t_amp: amplitude of T waves
% t_ind: index of T waves
...
% findpeaks(ecg_m,'MINPEAKDISTANCE',round(0.2*fs));

% findpeaks(ecg_m,'MINPEAKDISTANCE',round(0.2*fs));
% plot(round(0.2*fs),ecg_m,locs,pks,'o')
...
for i = 1 : length(pks)
    %disp(i)
    qrs_length_od=length(qrs_i_raw);
...
THRS_buf1 = [THRS_buf1 THR_SIG1];
 ...
 %% define other peaks
qrs_length_new=length(qrs_i_raw);
if qrs_length_new>qrs_length_od
    if qrs_length_new==1
        hri=qrs_i_raw(qrs_length_new);
    elseif qrs_length_new>1
        hri=qrs_i_raw(qrs_length_new)-qrs_i_raw(qrs_length_new-1);
    end
    
%     if RR inter much smaller than 8 average then search around 1/2 of R
%     peaks instead of 1/3
    if mean_RR-hri>50
        mean_int=round(hri/2);
    else
        mean_int=round(hri/3);
    end
%     q
    [qy_i,qx_i] = min(ecg_h(qrs_i_raw(qrs_length_new)-mean_int:qrs_i_raw(qrs_length_new)));
    qind_temp=qrs_i_raw(qrs_length_new)-mean_int+qx_i-1;
    qamp_temp=qy_i;
%     p
    [py_i,px_i] = max(ecg_h(qrs_i_raw(qrs_length_new)-mean_int:qind_temp));
    pind_temp=qrs_i_raw(qrs_length_new)-mean_int+px_i-1;
    pamp_temp=py_i;
%     s
    [sy_i,sx_i] = min(ecg_h(qrs_i_raw(qrs_length_new):min(qrs_i_raw(qrs_length_new)+mean_int,length(ecg_h))));
    sind_temp=qrs_i_raw(qrs_length_new)+sx_i-1;
    samp_temp=sy_i;
%     t
    [ty_i,tx_i] = max(ecg_h(sind_temp:min(qrs_i_raw(qrs_length_new)+mean_int,length(ecg_h))));
    tind_temp=sind_temp+tx_i-1;
    tamp_temp=ty_i;
    
    q_ind=[q_ind qind_temp];
    p_ind=[p_ind pind_temp];
    s_ind=[s_ind sind_temp];
    t_ind=[t_ind tind_temp];
    
    q_amp=[q_amp qamp_temp];
    p_amp=[p_amp pamp_temp];
    s_amp=[s_amp samp_temp];
    t_amp=[t_amp tamp_temp];
    %disp(i)
end
end
...
if gr
figure,plot(ecg_h);title('QRS on Filtered Signal');axis tight;
hold on,scatter(qrs_i_raw,qrs_amp_raw,'r','filled','o');
hold on,scatter(q_ind,q_amp,'g');
hold on,scatter(p_ind,p_amp,'m');
hold on,scatter(s_ind,s_amp,'b');
hold on,scatter(t_ind,t_amp,'k');
...
legend('filtered signal','R','Q','P','S','T','Noise level(black)','Signal level(red)','Adaptive Threshold (green)')
end
% 
% if gr
