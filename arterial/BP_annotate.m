...
%     global Fs;
    ...
    if length(unique(inWaveform)) < 10
        warning('No enough unique values for the wave trace.');
       footIndex = [];
       systolicIndex = [];
       notchIndex = [];
       dicroticIndex = [];
...
%     disp('Annotating blood pressure...')
    ...
    [bpwaveform, time, origtime] = BP_resample(inWaveform, inFs, Fs);
    % filter
    ...
    nan_ind = isnan(bpwaveform);
    bpwaveform(nan_ind) = mean(bpwaveform(~nan_ind));
    
    % add try catch when there is no foot found
    try
           footIndex = getFootIndex( bpwaveform, waveformDDPlus, zoneOfInterest );

    catch 
       warning('No foot index found.');
       footIndex = [];
       systolicIndex = [];
       notchIndex = [];
       dicroticIndex = [];
...
%     footIndex = getFootIndex( bpwaveform, waveformDDPlus, zoneOfInterest );
    ...
    [ dicroticIndex, notchIndex ] = getDicroticIndex( waveformDD, waveformD, bpwaveform, footIndex, systolicIndex, Fs );  
...
function [ dicroticIndex, notchIndex ] = getDicroticIndex( waveformDD, waveformD, bpwaveform, footIndex, systolicIndex, Fs )
%     global Fs;
